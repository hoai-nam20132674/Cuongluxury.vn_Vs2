...
@section('content')
	<div class="page-content ">
                    ...
                    <div id="main">
                        <form method="POST" action="{{URL::route('postAddBlogCategorie')}}" enctype="multipart/form-data" accept-charset="UTF-8" id="form_1aa3f76ebce588e61c3b18ff42edfa1a">
                            @if(isset($request->blog_cate_id))
                            <div class="note note-success">
                                @php
                                    if(isset($request->blog_cate_id)){
                                        $cate = App\BlogCate::where('id',$request->blog_cate_id)->get()->first();
                                    }
                                @endphp
                                @if($request->lang == 'vi')
                                    <p>Bạn đang thêm bản dịch "<strong class="current_language_text">Tiếng Việt</strong>" cho danh mục tin <a href="#">{{$cate->name}}</a></p>
                                @elseif($request->lang == 'en')
                                    <p>Bạn đang thêm bản dịch "<strong class="current_language_text">English</strong>" cho danh mục tin <a href="#">{{$cate->name}}</a></p>
                                @elseif($request->lang == 'ko')
                                    <p>Bạn đang thêm bản dịch "<strong class="current_language_text">Korean</strong>" cho danh mục tin <a href="#">{{$cate->name}}</a></p>
                                @elseif($request->lang == 'ja')
                                    <p>Bạn đang thêm bản dịch "<strong class="current_language_text">Japan</strong>" cho danh mục tin <a href="#">{{$cate->name}}</a></p>
                                @endif
