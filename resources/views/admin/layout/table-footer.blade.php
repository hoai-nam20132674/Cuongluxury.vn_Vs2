		...
		<ul class="pagination" request="{{$request->page}}">
			@if($items->currentPage() != 1)
				<li class="paginate_button page-item previous" id="table-products_previous">
					<a href="{{$items->url($items->currentPage()-1)}}&role={{$request->role}}" aria-controls="table-products" aria-label="&amp;laquo; Trước" data-dt-idx="0" tabindex="0" class="page-link">« Trước</a>
				</li>
			@else
			...
			@for($i =1; $i<=$items->lastPage();$i++)
				@if($request->page == $i)
					<li data-dt-idx="{{$i}}" class="paginate_button page-item active"><a href="{{$items->url($i)}}&role={{$request->role}}" aria-controls="table-products" data-dt-idx="{{$i}}" tabindex="0" class="page-link">{{$i}}</a></li>
				@else
					<li data-dt-idx="{{$i}}" class="paginate_button page-item"><a href="{{$items->url($i)}}&role={{$request->role}}" aria-controls="table-products" data-dt-idx="{{$i}}" tabindex="0" class="page-link">{{$i}}</a></li>
				@endif
			@endfor
			@if( $items->currentPage() != $items->lastPage())
				<li class="paginate_button page-item next" id="table-products_next">
					<a href="{{$items->url($items->currentPage()+1)}}&role={{$request->role}}" aria-controls="table-products" aria-label="Sau &amp;raquo;" data-dt-idx="3" tabindex="0" class="page-link">Sau »</a>
				</li>
			@else
