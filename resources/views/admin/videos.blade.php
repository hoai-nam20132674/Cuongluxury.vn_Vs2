...
@section('content')
	<div class="page-content ">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">Dang sách video</li>
		                ...
		                <div class="table-responsive  table-has-actions   table-has-filter ">
                            <div id="table-categories_wrapper" class="dataTables_wrapper container-fluid dt-bootstrap4 no-footer" style="width: 100%;">
                            	...
                            	<div class="dt-buttons btn-group"> 
                            		<button class="btn btn-secondary action-item" tabindex="0" aria-controls="table-categories"><span><span data-action="create" data-href="{{URL::route('addVideo')}}"><i class="fa fa-plus"></i> Tạo mới</span></span></button> 
                            	...
                            	<table class="table table-striped table-hover vertical-middle dataTable no-footer" id="table-categories" role="grid" aria-describedby="table-categories_info" style="width: 1582px;">
                            		<thead>
										<tr>
											...
											<th  title="Tên" width="200px" class="text-left column-key-name">Tên</th>
											<th  title="Categories" width="100px" class="no-sort column-key-updated_at">Danh mục</th>
										...
										@foreach($videos as $video)
											...
											@endif
													...
													<td class="  column-key-image">
														<img src="{{asset('uploads/images/videos/'.$video->avata)}}" width="100" alt="{{$video->name}}">
													</td>
													<td class=" text-left column-key-name"><a href="">{{$video->title}}</a></td>
													@php
													...
													<td class=" no-sort column-key-updated_at">
														@foreach($vcids as $vcid)
															@php
																$c = App\VideoCate::where('id',$vcid->cate_id)->get()->first();
															@endphp
															<a class="badge badge-warning">{{$c->name}}</a>
															
														@endforeach
													</td>
													<td class="  column-key-created_at">{{$video->created_at}}</td>
													@if($video->status ==1 )
													...
													<td class=" text-center">
														<div class="table-actions">
										                    ...
										                    <a href="{{URL::route('editVideo',$video->id)}}" class="btn btn-icon btn-sm btn-primary" data-toggle="tooltip" data-original-title="Sửa">
										                    	<i class="fa fa-edit"></i>
										                    </a>
										        
										                    <a href="#" class="btn btn-icon btn-sm btn-danger deleteDialog delete" data-toggle="tooltip" data-section="{{URL::route('deleteVideo',$video->id)}}" role="button" data-original-title="Xóa bản ghi">
