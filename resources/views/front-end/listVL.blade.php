...
@section('head')
	...
	<title>Danh sách gian hàng đăng ký tự nhiên</title>
	...
	<div class="container contaiver-v2">
		<nav class="b__breadcrumb--seller" aria-label="breadcrumb">
			<ol class="breadcrumb">
				...
				<li class="breadcrumb-item active" aria-current="page">Danh sách gian hàng tự nhiên</li>
			</ol>
		</nav>
								...
								<div class="b__seller--right b__seller--ls product-affiliate" ng-controller="create-o2o-campaign" ng-cloak>
									...
									<div class="b__seller--box">
										<div class="b__title title_filter">
											<div class="form-row" style="padding-bottom: 10px">
												<div class="col-md-6 col-sm-12">
													...
													@else
													<h3>Danh sách gian hàng vãng lai</h3>
													@endif
												</div>
												<div class="col-md-6 col-sm-12">
										...
										<div class="b__shop--info shop-own item-ctv" item-id="{{$user->id}}">
								                ...
								                <ul ng-controller="chatCtrl">
								                  ...
								                  <li>
								                  	@php
							        					...
							        					$admins = App\User::where('role',2)->get();
							        				@endphp
								                  	<div class="form-group" style="margin-bottom: 0px;">
				                                        <select class="form-control ui-select ui-select {{$user->id}}" onchange="updateVP('{{URL::route('updateVP',$user->id)}}',{{$user->id}})" name="vp" id="vp">
				                                        	<option value="">Khu vực quản lý</option>
				                                        	@foreach($admins as $admin)
													        	<option value="{{$admin->id}}">{{$admin->vp}}</option>
													        @endforeach
								          ...
								          <div class="b__shop--ls pd-top-17-mb shop-content-right" style="margin-right: 0; padding-right: 0;">
								            <div class="info-content-left">
								              <div class="info-content">
								                <label>Điểm thưởng:</label>
								                <strong class="merchant-point" id="{{$user->id}}" title="Thay đổi điểm thưởng" style="cursor: pointer;" onclick="updatePoint('{{$user->id}}','{{URL::route('merchantUpdatePoint',[$user->id,0])}}')">{{$user->point}}</strong>
								              </div>
								              <div class="info-content">
								                <label>Số Sản Phẩm:</label>
								                <strong>{{count($user->products)}}</strong>
								              </div>
								            </div>
								            <div class="info-content-right">
								              ...
								              <div class="info-content">
								                <label>Địa chỉ:</label>
								                <strong title="{{$user->address}}">{!! \Illuminate\Support\Str::words($user->address, 5,'...')  !!}</strong>
								              </div>
								            </div>
								          </div>
								        </div>
	...
	<script type="text/javascript">
	    ...
	    function updateVP(i,a){
		    ...
		    $.ajax({
	                ...
	                success: function(data) {
	                    if(data ==1 ){
	                    	toastr.success('Cập nhật khu vực quản lý cho gian hàng thành công');
	                    	$('.item-ctv[item-id="'+a+'"]').remove();
	                    }
	                    else{
	    ...
	    function updatePoint(userId,url){
	            swal("Nhập điểm thưởng", {
	              content: "input",
	            })
	            .then((value) => {
	              if($.isNumeric(value)){
	                ...
	                $.ajax({
	                    ...
	                    success: function(data) {
	                        if(data == 1){
	                            ...
	                            $('.merchant-point[id='+userId+']').html(value);
	                        }
	                        else{
	                ...
	                swal("Vui lòng nhập điểm dạng chữ số")
	                .then((value) => {
	                  updatePoint(userId,url);
	                });
